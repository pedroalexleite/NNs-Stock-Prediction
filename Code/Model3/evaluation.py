...
data_full = pd.read_csv('../Data/dataset4.csv')
...
metrics_df.to_csv('../Data/final_results.csv', index=False)
...
try:
    ...
    merged_df.to_csv('../Data/risk_return.csv', index=False)
    print("Risk-return analysis saved to risk_return.csv")
except Exception as e:
    print(f"Warning: Could not merge with sector data: {e}")
    risk_return_df.to_csv('../Data/risk_return.csv', index=False)
