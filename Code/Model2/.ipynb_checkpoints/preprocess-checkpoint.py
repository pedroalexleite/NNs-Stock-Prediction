...
def run_experiments_with_seeds(configurations, data_preparation_func, symbol='AAPL', seeds=BASE_SEEDS, 
                               look_back=30, nepochs=50, horizon=7, experiment_name="Experiment"):
    all_results = []
    ...
    for run_idx, seed in enumerate(seeds):
        ...
        for config_idx, config in enumerate(configurations):
            ...
            try:
                # Prepare data with the specific technique
                scaler, data_norm, train_norm, test_norm = data_preparation_func(
                    symbol=symbol, 
                    **config.get('technique_params', {})
                )
    ...
    #agregate results by configuration
    aggregated_results = {}
...
#data preparation wrapper functions
def prepare_data_outliers(symbol, **kwargs):
    if not kwargs:  # No outlier handling
