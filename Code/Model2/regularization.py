...
def model_with_regularization(scaler, data, train, test, look_back=30, nepochs=50, horizon=7, plot_predictions=False,
    ...
    if regularization_technique == 'l2_regularization':
        if activation == 'leaky_relu':
            x = GRU(8, kernel_regularizer=l2(reg_params.get('l2_lambda', 0.01)), return_sequences=True)(input_layer)
            x = LeakyReLU(alpha=0.01)(x)
            x = GRU(8, kernel_regularizer=l2(reg_params.get('l2_lambda', 0.01)), return_sequences=True)(x)
            x = LeakyReLU(alpha=0.01)(x)
            x = GRU(8, kernel_regularizer=l2(reg_params.get('l2_lambda', 0.01)))(x)
            x = LeakyReLU(alpha=0.01)(x)
        else:
            x = GRU(8, activation=activation, kernel_regularizer=l2(reg_params.get('l2_lambda', 0.01)), return_sequences=True)(input_layer)
            x = GRU(8, activation=activation, kernel_regularizer=l2(reg_params.get('l2_lambda', 0.01)), return_sequences=True)(x)
            x = GRU(8, activation=activation, kernel_regularizer=l2(reg_params.get('l2_lambda', 0.01)))(x)
        ...
        if activation == 'leaky_relu':
            x = GRU(8, recurrent_dropout=reg_params.get('recurrent_dropout_rate', 0.2), return_sequences=True)(input_layer)
            x = LeakyReLU(alpha=0.01)(x)
            x = GRU(8, recurrent_dropout=reg_params.get('recurrent_dropout_rate', 0.2), return_sequences=True)(x)
            x = LeakyReLU(alpha=0.01)(x)
            x = GRU(8, recurrent_dropout=reg_params.get('recurrent_dropout_rate', 0.2))(x)
            x = LeakyReLU(alpha=0.01)(x)
        else:
            x = GRU(8, activation=activation, recurrent_dropout=reg_params.get('recurrent_dropout_rate', 0.2), return_sequences=True)(input_layer)
            x = GRU(8, activation=activation, recurrent_dropout=reg_params.get('recurrent_dropout_rate', 0.2), return_sequences=True)(x)
            x = GRU(8, activation=activation, recurrent_dropout=reg_params.get('recurrent_dropout_rate', 0.2))(x)
    ...
    if regularization_technique == 'dropout':
        x = Dropout(0.1)(x)
        print("Applied Dropout with rate: 0.1 after GRU layers")

    elif regularization_technique == 'batch_normalization':
        x = BatchNormalization()(x)
        print("Applied Batch Normalization after GRU layers")
